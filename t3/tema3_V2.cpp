...
public:
    ...
    void aprindePixel(int coord_x, int coord_y) {
        drawCircleMethod2(this->radius, coord_x * dl, coord_y * dc);
    }
    ...
    void trasareSegment(float x0, float y0, float xn, float yn, int width) {
        for (auto& points : AfisareSegmentDreapta3(x0, y0, xn, yn)) {
            aprindePixel(points.first, points.second );
        }
        glBegin(GL_LINES);
...
void Display()
{
    ...
    gc.trasareSegment(-7.6, 7.6, 8, 2.4, 3);
    glFlush();
}
